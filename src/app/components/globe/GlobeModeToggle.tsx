...
export const GlobeModeToggle = ({ mode, onModeChange }: GlobeModeToggleProps) => {
  return (
    <div className="glass-panel absolute left-4 top-4 z-20 w-[min(21rem,calc(100%-2rem))] rounded-[1.35rem] p-3 shadow-2xl md:left-6 md:top-6">
        ...
        {modes.map((item) => (
          <button
            ...
            className={`rounded-xl px-3 py-2 text-sm transition-all duration-300 ${
              mode === item.value
                ? "border border-[#f4c430]/35 bg-[linear-gradient(135deg,rgba(244,196,48,0.24),rgba(29,73,216,0.16))] text-white shadow-[0_12px_26px_rgba(0,0,0,0.22)]"
